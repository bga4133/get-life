...
import { AlbumListProps } from '../models/types/AlbumListProps'
import { Users } from '../models/types/Users';
export const AlbumsList = ({ id, userId , title, usersLists, photosList }: AlbumListProps) => {
  const [users, setUsers] = useState<any>([]);  
  const [photosLength, setPhotosLength] = useState<any>([]);  
  ...
  const showAuthors = () => {
    ...
    const photosByAlbum = () => {
      const photosResult = photosList?.filter(item => item.albumId === id)
      setPhotosLength(photosResult)
      setShowItemDetail(true);
    }
    const closeModal = () => {
  ...
  return (
    ...
    <div className="mt-10 ml-1 md:ml-5 mt-6 lg:ml-5 p-6 max-w-sm bg-white rounded-lg border border-gray-200 shadow-md dark:bg-gray-800 dark:border-gray-700">
        ...
        <h1 className="mb-3 text-white text-xl font-semibold dark:text-white"><span>Title: </span>{title}</h1>
        <button
          className="inline-flex items-center py-2 px-3 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
          onClick={photosByAlbum}
        >
          Photos the album
          <svg
            aria-hidden="true"
        ...
        <button
          className="inline-flex ml-2 items-center py-2 px-3 text-sm font-medium text-center text-white bg-blue-700 rounded-lg hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 dark:bg-blue-600 dark:hover:bg-blue-700 dark:focus:ring-blue-800"
          onClick={showAuthors}
        >
          Show Authors
          <svg
      ...
      {itemDetail && (
        <>
          ...
          <p className="mb-3 font-normal text-gray-700 dark:text-gray-400">
            {photosLength !== null && <>{photosLength ? photosLength.length : ''} Photos</>}
          </p>
        </>
        )}
      </div>
